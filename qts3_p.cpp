...
QtS3ReplyPrivate *QtS3Private::listObjects(const QByteArray &bucket, const QByteArray &prefix, const QByteArray &delimiter)
{
    QtS3ReplyPrivate *s3Reply = new QtS3ReplyPrivate;

    if (!checkBucketName(s3Reply, bucket))
        return s3Reply;
    if (!cacheBucketLocation(s3Reply, bucket))
        return s3Reply;

    QNetworkReply *networkReply = sendS3Request(bucket, "GET", QByteArray(), QString("list-type=2&prefix="+prefix+"&delimiter="+delimiter).toUtf8(),
                                                QByteArray(), QStringList());
...
QtS3ReplyPrivate *QtS3Private::copyFile(const QByteArray &bucket, const QString & oldName, const QString &newName)
{
    QtS3ReplyPrivate *s3Reply = new QtS3ReplyPrivate;
    ...
    QStringList headers;
    headers<<"x-amz-copy-source:/"+bucket+"/"+oldName;
    QNetworkReply *networkReply = sendS3Request(bucket, "PUT", newName, QByteArray(),
                                                QByteArray(), headers);
